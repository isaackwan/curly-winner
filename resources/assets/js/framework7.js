...
window.myApp.onPageInit('transactions/index', function (page) {
	$$('a.tab-link').removeClass('active');
	$$('a[href="/transactions"].tab-link').addClass('active');
});

import LoanCalc from 'loan-calc';
window.myApp.onPageInit('loans/show', function(page){
	window.calculatorVue = new Vue({
		el: '#calculatorVue',
		data: {
			amount: 10000,
			termMonths: 24,
		...
		mounted: function() {
			this.rate = this.$el.attributes.rate.value;
			console.log("mounted #calculatorVue");
		}
	});
});
window.myApp.onPageInit('advices', function(page) {
	var iCurrentSpeed = 80,
		iTargetSpeed = 80,
		bDecrement = null,
		job = null;
	...
	function draw() {
		...
		var canvas = document.getElementById('tutorial-speedometer'),
			options = null;
...
/*window.$$(document).on('page:init', function(){
